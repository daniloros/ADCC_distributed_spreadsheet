...
add_row(NameSheet, Tab) ->
  ...
  case mnesia:dirty_read(tab, TabName) of
    [{tab, _, CellIds}] ->
      %% estraggo il numero di indice delle colonne
      ColumnsIndex = [Column || {_,_, Column} <- CellIds],
      %% estraggo il numero di indice delle righe
      RowNumbers = [Row || {_, Row, _} <- CellIds],

      %%% ad esempio:
      %% mi resituirà una lista fatta in questo modo: [1,1,1,2,2,2,3,3,3,4,4,4,5,5,5,6,6,6]
      %% devo quindi eliminare i duplicati
      NewRowIndex = length(lists:usort(RowNumbers)) +1, %% ottengo il numero della nuova riga aggiungendo +1
      NumColumns = length(lists:usort(ColumnsIndex)), %% il numero delle colonne sarà lo stesso
      ...
      %% genero l'elenco che contiene gli id delle nuove celle aggiunte
      NewCellIds = lists:foldl(
        ...
        [],
        lists:seq(1, NumColumns)),
      %% aggiorno la tabella tab, includendo i nuovi id
...
delete_cell(CellIds, Index) ->
  ...
  CellIdsFiltered = lists:filter(
    ...
    CellIds),
  %% Elimina completamente le celle dal database
  F = fun() ->
    lists:foreach(
      fun(IdsFilter) ->
