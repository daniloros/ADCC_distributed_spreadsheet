...
-export([init/0, start/0, populate_cell/4, new/1, new/4, share/2, get_sheet/1,
  set_cell_value/5, get_cell_value/4, get_all_cell_values/1]).
...
%% Funzione per ottenere tutti i valori delle celle per uno specifico foglio di calcolo
...
%% ottengono il valore delle celle per ogni tab del foglio
get_cell_values(SpredsheetName, PageName) ->
%% io:format("PageName ~p\n", [PageName]),
  NewPageName = lists:concat([atom_to_list(SpredsheetName), integer_to_list(PageName)]),
%%  io:format("PageName ~p\n", [NewPageName]),
  case mnesia:dirty_read(sheet_page, NewPageName) of
    [{sheet_page, _, CellIds}] ->
%%      io:format("CellIds ~p \n", [CellIds]),
      [{PageName, NewPageName, CellValue} || CellId <- CellIds, CellValue <- [get_cell_value(CellId)]];
%%      [{PageName, CellValue} || CellId <- CellIds, CellValue <- [get_cell_value(CellId)]];
    _ ->
      []
  end.

%% ottengo il valore di una singola cella
get_cell_value(CellId) ->
  {_, Row, Column} = CellId,
%% io:format("CellId ~p\n", [CellId]),
  case mnesia:dirty_read(cell, CellId) of
    [{cell, _, _, _, Value}] ->
      {{Row, Column}, Value};
%%      Value;
    _ ->
...
%%% ritorno i valori delle celle però leggo che un file .CSV è fatto ad esempio in questo modo:
%%Sheet1
%%1,Alice,Smith
%%2,Bob,Johnson
%%3,Charlie,Brown

%%% quindi devo far in modo che i dati tornati siano come una matrice per righe e colonne dove c'è il valore
